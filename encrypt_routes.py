...
- API 버전 번호 포함 (서버 EXE 버전과 별도 관리)

사용법:
    python encrypt_routes.py <API_VERSION>
    python encrypt_routes.py 1.0.0
    python encrypt_routes.py 1.0.1 --output dist/mssql_routes.enc

버전 관리:
    - APP_VERSION (config.py): 서버 EXE 버전 (예: 1.5.6)
    - API_VERSION (enc 파일): MSSQL API 버전 (예: 1.0.0)
"""
...
def main():
    ...
    parser.add_argument("version", help="API version number (e.g., 1.0.0) - separate from server EXE version")
    ...
    try:
        ...
        print(f"Note: API version is managed separately from server EXE version (APP_VERSION in config.py)")
